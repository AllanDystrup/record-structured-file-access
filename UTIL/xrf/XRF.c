/**********************************************************
      ...
      with enhancements and Lattice compiler support in Dec. 1983.
              David N. Smith
              44 Ole Musket Lane
              Danbury, CT 06810
              (203) 748-5934
      ...
      PC Enhancements include:
        ...
        // -------------------------------------------------------
        Changes (no copyright) Feb. 2026 by Allan Dystrup [AD]
        Ported to C11 on WSL Ubuntu LINUX:
            1) #included some C11 header files
            2) defined function prototypes
            3) increased some const values (MAX_REF, MAX_ALPHA)
            4) some optional DEBUG output to troubleshoot faults
            5) removed deprecated 'Lattice' compiler support
            5) a bit of code reformatting, to make it (sort of) readable
            DISCLAIMER: The code does what is described in the Abstract
            BUT: It needs restructuring and robust memory handling!
        // --------------------------------------------------------
**+2 **************************************************************
	XRF.C
       	Abstract:
      	'XRF.C' is a cross-reference utility for 'C' programs.
...
// Const Values (Redefined [AD]) -----------------------------------------
#define  ERROR      	-1
#define  MAX_REF     	200  	/* maximum refs per ref-block */
#define  MAX_LEN     	30      /* maximum identifier length  */
#define  MAX_WRD    	2000    /* maximum number of identifiers */
#define  MAX_ALPHA  	500     /* maximum alpha chain heads */
#define  REFS_PER_LINE  15      /* output formatting: maximum refs per line */
#define  LINES_PER_PAGE 60	/* output formatting */
#define  MAXCOL     	110     /* default maximum column number for listing line */
...
char*   MyAlloc(int i);   // malloc "i" # bytes



// Function prototypes (AD) -------------------------------------------------
void    use_err();
char*   MyAlloc(int i);             // malloc "i" # bytes
int     proc_file(char*, int);
void    prnt_tbl();
int     lst_err();
void    get_include_fileid(char*, FILE*);
int     chk_token(char*);
void    nl();
void    prt_hdr();
int     get_token(FILE*, char*, int*, int*, int);
void    put_token(char*, int);
void    echo(char);
void    echochar(char);
char    rdchr(FILE*, int*, int);
int     fil_chr(FILE*, int*);
void    chain_alpha(struct id_blk*, char*);
int     abs(int);
...
struct id_blk *alloc_id(aid_token)
  char  *aid_token;
  {
    ...
    aid_ptr = (struct id_blk *) MyAlloc(sizeof(struct id_blk));
    if( aid_ptr == 0) {
...
struct rf_blk *alloc_rf(arf_ref)
  int  arf_ref;
  {
    ...
    if((arf_ptr = (struct rf_blk *) MyAlloc(sizeof(struct rf_blk))) == 0) {
